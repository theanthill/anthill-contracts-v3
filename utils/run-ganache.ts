...
async function runLocalNode(network: NetworksType, blocktime: number | undefined) {
    const ganacheArgs = ["ganache-cli"];

    const config = getChainConfig(network) as HttpNetworkConfig;
    ganacheArgs.push(...["-f", config.url, "--chainId", String(config.chainId)]);
    ...
    const ganache = spawn("ganache-cli", ganacheArgs, { stdio: [process.stdin, process.stdout, process.stderr] });
